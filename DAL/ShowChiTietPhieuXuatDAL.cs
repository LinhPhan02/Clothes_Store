...
namespace DAL
{
    public class ShowChiTietPhieuXuatDAL
    {
        public static List<ChiTietPhieuDTO> getDataChiTietPX(int id)
        {
            ...
            MySqlCommand cmd = new MySqlCommand($"SELECT p.name, dt.prices,dt.quantity,dt.prices*dt.quantity as intomoney FROM billexport bi, detailbillexport dt, products p, detailproduct as dtp WHERE bi.id = dt.order_id and bi.id = {id} and p.id = dtp.product_id and dtp.id = dt.product_id", conn);
