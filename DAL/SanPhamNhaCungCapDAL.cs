...
namespace DAL
{
    public class SanPhamNhaCungCapDAL
    {
        public static List<PProducerDTO> GetData()
        {
            ...
            MySqlCommand cmd = new MySqlCommand("SELECT pp.id, pp.name, pp.category_id, pp.origin, pp.prices, pp.producer_id, pp.image, pp.createdAt, pp.updatedAt FROM productproducers as pp,producers as p where pp.producer_id = p.id and p.status = 1 ", conn);
