...
namespace DAL
{
    public class SpQLSPDAL
    {
        ...
        public static List<SpQLSPDTO> GetDataAction()
        {
            List<SpQLSPDTO> productQL = new List<SpQLSPDTO>();
            MySqlConnection conn = SqlConnectionData.Connect();
            conn.Open();

            MySqlCommand cmd = new MySqlCommand("SELECT * ,temp.name as name_producer,temps.name as category_name, COUNT(detailproduct.id) as qty FROM `products`,(SELECT producers.id,producers.name FROM `producers`) as temp,(SELECT category.id,category.name FROM `category`) as temps, detailproduct WHERE products.producer_id=temp.id and temps.id = products.category_id and detailproduct.product_id = products.id and detailproduct.isActive = 1 group by products.id", conn);
