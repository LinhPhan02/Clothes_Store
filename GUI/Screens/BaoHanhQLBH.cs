...
namespace GUI.Screens
{
    public partial class BaoHanhQLBH : UserControl
    {
        ...
        public BaoHanhQLBH()
        {
            InitializeComponent();
            initDatagridView();
            if (Login._checkUrlMatch("thembaohanh:QLBaoHanh"))
            {
        ...
        {
            if (txt_search_ncc.Text == "Tìm kiếm theo ID product")
            {
                txt_search_ncc.Text = "";
        ...
        private void AddText(object sender, EventArgs e)
        {
            ...
            {
                txt_search_ncc.Text = "Tìm kiếm theo ID product";
        ...
        private void dgv_ncc_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            bool isUpdate = false;
            bool isdel = false;
            if (Login._checkUrlMatch("suabaohanh:QLBaoHanh"))
            {
                isUpdate = true;
            }
            else
            ...
            if (Login._checkUrlMatch("xoabaohanh:QLBaoHanh"))
            {
                isdel = true;
            }
            else
            ...
            XacNhanThucHien confirm = new XacNhanThucHien(maintains[e.RowIndex].Id, isUpdate, isdel);
            confirm.ShowDialog();
        ...
        {
            if (txt_search_ncc.Text == "" || txt_search_ncc.Text == "Tìm kiếm theo ID product")
            {
                loadDataToDataGridView(maintains);
                return;
            }
            foreach (BaoHanhDTO item in maintains)
            {
                 if (item.ProductId.ToString().IndexOf(txt_search_ncc.Text) != -1)
                {
