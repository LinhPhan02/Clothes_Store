...
namespace GUI.Screens
{
    public partial class QuanLyDanhMuc : UserControl
    {
        ...
        private void dataGridView_danhmuc_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            bool isUpdate = false;
            bool isdel = false;
            if (Login._checkUrlMatch("suadanhmucsanpham:QLDM"))
            {
                isUpdate = true;
            }
            else
            ...
            if (Login._checkUrlMatch("xoadanhmucsanpham:QLDM"))
            {
                isdel = true;
            }
            else
            ...
            XacNhanThucHien action = new XacNhanThucHien(Convert.ToInt32(row.Cells["Id"].Value), isUpdate, isdel);
            action.ShowDialog();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            foreach (DanhMucDTO dm in list)
            {
                ...
                else if (dm.Name.ToLower().Contains(this.txt_search.Text.ToLower()))
                {
                    list_search.Add(dm);
                }
            }
