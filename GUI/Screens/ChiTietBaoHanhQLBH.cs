...
namespace GUI.Screens
{
    public partial class ChiTietBaoHanhQLBH : UserControl
    {
        ...
        private void loadDataToDataGridView(List<ChiTietBaoHanhDTO> details)
        {
            ...
            {
                string[] row = new string[] { item.Id.ToString(), item.DetailProductId.ToString(), $"{item.CustomerId}", item.CreatedAt.ToString("dd-MM-yyyy hh:mm:ss"), item.OutOfDate.ToString("dd-MM-yyyy hh:mm:ss"), item.Status == 1 ? "Đang bảo hành" : "Chưa bảo hành" };
        ...
        {
            if (Login._checkUrlMatch("suachitietbaohanh:QLBaoHanh"))
            {
                ...
                dgv_ncc.Rows[e.RowIndex].Cells[5].Value = update.detail.Status == 1 ? "Đang bảo hành" : "Chưa bảo hành";
